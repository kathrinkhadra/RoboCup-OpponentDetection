...
def main():
    ...
    for n in range(0, len(fileNames)):
            if fileNames[n]=="template.png":
                imq=cv2.imread(join(mypath,fileNames[n]))
    for n in range(0,len(fileNames)):
...
#        imq = imd[n].copy()
    ...
    topFourImages(templateTopFour, fileNames, "template.png", "Template Matching")
    imageScorePlacing(templateMatchingScores, fileNames, "template.png", "Template Matching")

    histogramTopFour, histogramMatchingScores = histogramMatching(imd, imq, n)
    topFourImages(histogramTopFour, fileNames, "template.png", "Histogram Matching")
    imageScorePlacing(histogramMatchingScores, fileNames, "template.png", "Histogram Matching")

    siftTopFour, siftMatchingScores = SIFT(imd, imq, n)
    topFourImages(siftTopFour, fileNames, "template.png", "SIFT")
    imageScorePlacing(siftMatchingScores, fileNames, "template.png", "SIFT")
...
def topFourImages(topFour, fileNames, image, method):
    print(image + " top matches in: " + method)
...
def imageScorePlacing(allScores, fileNames, image, method):
    print(image + " all scores in: " + method)
    count = len(allScores)
